...
export class AuroraOpportunitiesJob {
  ...
  constructor() {
    // Queue configuration
    this.queue = new Queue<OpportunitiesJobData>('aurora-opportunities', {
    ...
    logger.info('Aurora Opportunities Job initialized');
  }
  ...
  private async scheduleRecurringOpportunities(): Promise<void> {
    await this.queue.add(
      ...
      {} as any,
      {
        ...
        jobId: 'aurora-opportunities-trigger'
      }
    );
  ...
  private async processOpportunities(job: Job<OpportunitiesJobData>): Promise<void> {
    ...
    try {
      ...
      // Identify opportunities using Aurora
  ...
  public async triggerAllOpportunities(): Promise<void> {
    try {
      ...
      // Queue report for each owner
      for (const owner of owners) {
        const org = owner.organizations as any;
        ...
        await this.queue.add('send-opportunities', {
          ...
          ownerName: owner.owner_name || 'Dono',
  ...
  public async triggerOpportunities(organizationId: string): Promise<void> {
    try {
      ...
      if (ownerError || !owner) {
        throw new Error('No active owner found for organization');
      }
  ...
  public async close(): Promise<void> {
    ...
    logger.info('Aurora Opportunities Job closed');
  }
}
