  ...
  async (req: TenantRequest, res: Response): Promise<void> => {
    try {
      ...
      const reservation = await DaycareService.createStay({
        ...
        checkInDate,
        checkOutDate,
        notes: [specialRequests, medicalNotes].filter(Boolean).join('\n\n'),
 ...
 * Query params: status?, petId?, startDate?, endDate?
 */
router.get(
  '/reservations',
  readLimiter,
  async (req: TenantRequest, res: Response): Promise<void> => {
    try {
      const organizationId = req.organizationId!;
      const { status, petId, startDate, endDate, limit, offset } = req.query;
      ...
      // Filter by petId if provided (client-side filter as service doesn't support it)
      let filteredReservations = result.stays;
      if (petId) {
        filteredReservations = filteredReservations.filter((stay: any) => stay.pet_id === petId);
      }

      res.json({
        success: true,
        reservations: filteredReservations,
        total: petId ? filteredReservations.length : result.total,
        limit: result.limit,
...
router.put(
  ...
  async (req: TenantRequest, res: Response): Promise<void> => {
    try {
      ...
      if (req.body.notes || req.body.specialRequests || req.body.medicalNotes) {
        updates.notes = [
          req.body.notes,
  ...
  async (req: TenantRequest, res: Response): Promise<void> => {
    try {
      ...
      const checkOutNotes = [
        ...
        petCondition ? `Pet Condition: ${petCondition}` : null,
