...
/**
 * POST /api/internal/clients/:clientId/aurora-message
 * Envia mensagem via Aurora (God Mode)
 */
router.post(
  '/clients/:clientId/aurora-message',
  requireAdminRole(['super_admin']),
  async (req: AdminRequest, res: Response): Promise<void> => {
    try {
      const { clientId } = req.params;
      ...
      logger.info({
        ...
        adminId: req.admin?.id
      }, 'Admin sent Aurora message');

      res.json({
        success: true,
        message: 'Mensagem enviada via Aurora',
        to: ownerNumber.phone_number
      });
    } catch (error: any) {
      logger.error({ error, clientId: req.params.clientId }, 'Error sending Aurora message');
...
router.post(
  ...
  async (req: AdminRequest, res: Response): Promise<void> => {
    try {
      ...
      const validFeatures = ['training', 'daycare', 'bipe', 'aurora-enhanced'];
