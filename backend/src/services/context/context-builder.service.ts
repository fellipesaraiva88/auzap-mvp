...
export interface ClientContext {
  ...
  bookings: {
    recent: Tables<'bookings'>[];
    upcoming: Tables<'bookings'>[];
  };
}

/**
 * Serviço responsável por construir contexto enriquecido para processamento de IA
 * Busca últimas 5 mensagens + dados do contato + pets + bookings
 */
export class ContextBuilderService {
  ...
  async buildClientContext(
    ...
    try {
      ...
      const [contact, pets, messages, recentBookings, upcomingBookings] = await Promise.all([
  ...
  /**
   * Busca pets do contato (SEMPRE validando organization_id)
   */
  private async getPets(
    ...
    if (error) {
      logger.error({ error, contactId }, 'Error fetching pets');
      return [];
    }
  ...
  /**
   * Busca bookings recentes (últimos 30 dias)
   */
  private async getRecentBookings(
    ...
    if (error) {
      logger.error({ error, contactId }, 'Error fetching recent bookings');
      return [];
    }
  ...
  private async getUpcomingBookings(
    ...
    if (error) {
      logger.error({ error, contactId }, 'Error fetching upcoming bookings');
      return [];
    }
  ...
  formatContextForPrompt(context: ClientContext): string {
    ...
    if (context.pets.length > 0) {
      prompt += `\nPets cadastrados (${context.pets.length}):\n`;
      context.pets.forEach(pet => {
        const age = pet.age_years
          ? `${pet.age_years} ano(s)${pet.age_months ? ` e ${pet.age_months} mês(es)` : ''}`
          : pet.age_months
            ? `${pet.age_months} mês(es)`
            : 'idade não informada';

        prompt += `  • ${pet.name} - ${this.translateSpecies(pet.species)}`;
    ...
    // Bookings recentes
    if (context.bookings.recent.length > 0) {
      prompt += `\nAgendamentos recentes (${context.bookings.recent.length}):\n`;
      context.bookings.recent.forEach(booking => {
        const date = new Date(booking.scheduled_start).toLocaleDateString('pt-BR');
        prompt += `  • ${date} - Status: ${this.translateStatus(booking.status)}\n`;
      });
    }

    // Bookings futuros
    if (context.bookings.upcoming.length > 0) {
      prompt += `\nAgendamentos futuros (${context.bookings.upcoming.length}):\n`;
      context.bookings.upcoming.forEach(booking => {
        const date = new Date(booking.scheduled_start).toLocaleDateString('pt-BR');
        const time = new Date(booking.scheduled_start).toLocaleTimeString('pt-BR', {
          hour: '2-digit',
          minute: '2-digit'
        });
        prompt += `  • ${date} às ${time} - Status: ${this.translateStatus(booking.status)}\n`;
      });
    }
