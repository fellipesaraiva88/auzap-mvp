...
export class BookingsService {
  ...
  async findById(bookingId: string): Promise<Booking | null> {
    ...
    if (error) {
      logger.error({ error, bookingId }, 'Error finding booking by ID');
      return null;
    }
  ...
  async listByOrganization(
    ...
    if (error) {
      logger.error({ error, organizationId }, 'Error listing bookings');
      return [];
    }
   ...
   */
  async listToday(organizationId: string): Promise<Booking[]> {
   ...
   */
  async update(bookingId: string, data: UpdateBookingData): Promise<Booking | null> {
    ...
    logger.info({ bookingId, status: data.status }, 'Booking updated');
  ...
  async delete(bookingId: string): Promise<void> {
    ...
    logger.info({ bookingId }, 'Booking deleted');
  }
}
