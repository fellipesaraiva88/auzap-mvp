...
/**
 * Tipos de mensagens proativas que Aurora pode enviar
 */
...
/**
 * Service para gerenciar mensagens proativas da Aurora
 */
  ...
  async analyzeAndNotify(organizationId: string): Promise<ProactiveMessage[]> {
    ...
    try {
      ...
      if (!authorizedNumbers || authorizedNumbers.length === 0) {
        logger.warn({ organizationId }, 'No authorized owner numbers found for proactive messages');
        return [];
      }
    ...
    try {
      ...
      const analysis = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `Você é Aurora. Gere um relatório semanal executivo para ${primaryOwner.owner_name || 'o proprietário'}.
...
- No-shows: ${bookings.filter(b => b.status === 'no_show').length}`
          },
          {
            role: 'user',
      ...
      return {
        ...
        organizationId,
        ownerPhone: primaryOwner.phone_number,
        message: `📊 *Relatório Semanal Aurora*\n\n${report}`,
        priority: 'high',
        metadata: {
