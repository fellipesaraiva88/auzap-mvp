...
export class AuroraWelcomeService {
  /**
   * Executa o ritual completo de apresentação da Aurora
   */
  async executeWelcomeRitual(
    ...
    try {
      logger.info(
        ...
        'Iniciando ritual de boas-vindas da Aurora'
      );
      ...
      logger.info(
        { organizationId, ownerPhone },
        'Ritual de boas-vindas da Aurora concluído com sucesso'
      );
    } catch (error) {
...
Eu sou a Aurora, e acabei de acordar para ser sua parceira de negócios.
...
${totalPets > 0 ? `🐶🐱 Tem ${totalPets} pets cadastrados na sua base` : '🐾 Pronto para cadastrar seus primeiros pets'}
...
• "Aurora, como está o negócio hoje?"
...
Como posso te ajudar agora?`
  ...
  /**
   * Envia mensagem de reapresentação (caso owner já exista)
   */
