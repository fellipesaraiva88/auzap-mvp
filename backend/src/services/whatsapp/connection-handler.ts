...
export class ConnectionHandler {
  ...
  async handleQRCode(
    ...
    await this.updateInstanceStatus(instanceId, 'qr_pending', undefined, qrCode);
  ...
  async handlePairingCode(
    ...
    await this.updateInstanceStatus(instanceId, 'pairing_pending');
  ...
  private async updateInstanceStatus(
    ...
    try {
      ...
      logger.error({ error, instanceId }, 'Failed to update instance status');
    }
  }
  ...
  private emitConnectionEvent(event: ConnectionUpdateEvent): void {
    if (this.socketIOEmitter) {
      ...
      this.socketIOEmitter(`${namespace}:whatsapp:status`, event);
    }
  }
