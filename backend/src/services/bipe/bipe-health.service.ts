...
export type BipeProtocol = Tables<'bipe_protocol'>;
...
/**
 * BipeHealthService - BIPE Protocol for Pet Health Management
 *
 ...
 */
export class BipeHealthService {
  /**
   * Create initial BIPE protocol for a pet
   */
  async createProtocol(data: CreateProtocolData): Promise<BipeProtocol> {
    try {
      ...
      const protocolData: TablesInsert<'bipe_protocol'> = {
      ...
      if (error || !protocol) {
        throw error || new Error('Failed to create BIPE protocol');
      }

      logger.info({
        petId: data.petId,
        protocolId: protocol.id
      }, 'BIPE protocol created');

      // Generate initial alerts if needed
      await this.generateAutomaticAlerts(protocol.id, data.organizationId);

      return protocol as BipeProtocol;
    } catch (error) {
      logger.error({ error, data }, 'Error creating BIPE protocol');
      throw error;
    }
  }
   ...
   */
  async getProtocol(petId: string): Promise<BipeProtocol | null> {
    try {
      ...
      logger.error({ error, petId }, 'Error getting BIPE protocol');
      throw error;
    }
  }
  ...
  async updateBehavioralScore(
    ...
    score: number,
    notes?: string
  ): Promise<BipeProtocol | null> {
    try {
      this.validateScore(score);

      const protocol = await this.getProtocol(petId);
      if (!protocol) {
        throw new Error('BIPE protocol not found for pet');
      }

      const updateData: TablesUpdate<'bipe_protocol'> = {
   ...
   */
  async getProtocolHistory(petId: string): Promise<BipeProtocol[]> {
    try {
      const { data, error } = await supabaseAdmin
        .from('bipe_protocol')
        .select('*')
...
export const bipeHealthService = new BipeHealthService();
