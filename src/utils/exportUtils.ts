...
export async function exportPDF(
  ...
  html += `
      <div class="footer">
        <p>Gerado em ${new Date().toLocaleString("pt-BR")} | AuZap - Sistema de Gestão</p>
      </div>
    </body>
    </html>
  `;
...
// Função para exportar dados de pets com formatação específica
export function exportPets(
  ...
  const formattedData = pets.map(pet => ({
    ...
    "Idade": pet.age_years ? `${pet.age_years} anos` : "Não informada",
    "Peso": pet.weight_kg ? `${pet.weight_kg} kg` : "Não informado",
    "Dono": pet.owner_name || "",
    "Telefone Dono": pet.owner_phone || "",
    "Observações": pet.notes || "",
    "Ativo": pet.is_active ? "Sim" : "Não"
  }));

  return exportData(formattedData, format, {
    ...options,
    filename: options.filename || `pets-${new Date().toISOString().split("T")[0]}`
  });
}
