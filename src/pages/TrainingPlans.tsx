...
export default function TrainingPlans() {
  ...
  const handleCreate = async () => {
    if (!formData.pet_id || !formData.start_date) {
      toast({
        variant: "destructive",
        title: "Campos obrigatórios",
        description: "Selecione pet e data de início",
      });
      return;
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        ...
        subtitle="Gerencie planos de treino personalizados para cada pet"
        action={
              ...
              {plans.map((plan: any) => (
                <div key={plan.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-semibold">{plan.pet?.name}</h4>
