...
export interface AIActivity {
  id: string;
  type: 'booking' | 'sale' | 'contact' | 'pet' | 'message';
  description: string;
  timestamp: string;
  metadata?: {
    clientName?: string;
      ...
      return interactions.map((interaction: any) => {
        ...
        switch (interaction.function_name) {
          case 'criar_agendamento':
            type = 'booking';
            description = `Agendou ${args.service_type || 'serviço'} para ${args.pet_name || 'pet'}`;
            break;
          ...
          case 'cadastrar_pet':
            type = 'pet';
            description = `Cadastrou ${args.name || 'pet'} (${args.species || 'animal'})`;
            break;
          default:
            type = 'message';
