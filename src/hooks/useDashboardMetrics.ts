...
async function calculateGuaranteedRevenue(organizationId: string): Promise<{ amount: number; bookings: number }> {
  const today = new Date();
...
export function useDashboardMetrics() {
  ...
  useEffect(() => {
    ...
    on('booking:created', handleRealtimeUpdate);
    on('booking:updated', handleRealtimeUpdate);
    ...
    return () => {
      ...
      off('booking:created', handleRealtimeUpdate);
      off('booking:updated', handleRealtimeUpdate);
    };
