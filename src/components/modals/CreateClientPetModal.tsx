...
export function CreateClientPetModal({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Criar pets
      for (const pet of pets.filter((p) => p.name)) {
      ...
      toast({
        ...
        description: `${clientData.full_name} foi adicionado com ${pets.length} pet(s)`,
      });
  ...
  const steps = [
    ...
    { number: 2, title: "Adicionar Pets", icon: Dog },
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <p className="text-sm text-muted-foreground">
                  Adicione os pets do cliente (opcional)
                </p>
              ...
              {pets.length === 0 ? (
                <div className="border-2 border-dashed border-muted rounded-lg p-8 text-center">
                  ...
                  <p className="text-muted-foreground">Nenhum pet adicionado</p>
                  ...
                  {pets.map((pet, idx) => (
                    <Card key={pet.id} className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        <Badge variant="outline">Pet {idx + 1}</Badge>
                        <Button
                          size="icon"
                          variant="ghost"
