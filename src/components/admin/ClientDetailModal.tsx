...
export default function ClientDetailModal({
  ...
  const handleForceReconnect = async () => {
    ...
    try {
      ...
      toast.error('Erro ao forçar reconexão: ' + (error.response?.data?.error || 'Tente novamente'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleChangePlan = async () => {
    ...
    try {
      ...
      toast.error('Erro ao alterar plano: ' + (error.response?.data?.error || 'Tente novamente'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleInjectCredits = async () => {
    ...
    try {
      ...
      toast.error('Erro ao injetar créditos: ' + (error.response?.data?.error || 'Tente novamente'));
    } finally {
      setIsLoading(false);
    }
  };

  // 🚀 AÇÃO 4: Enviar Mensagem via Aurora (God Mode)
  const handleSendAuroraMessage = async () => {
    ...
    try {
      await axios.post(
        `${API_URL}/api/internal/clients/${client.id}/aurora-message`,
        { message: auroraMessage },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      toast.success('🤖 Mensagem Aurora enviada com sucesso!');
      setAuroraMessage('');
    } catch (error: any) {
      toast.error('Erro ao enviar mensagem: ' + (error.response?.data?.error || 'Tente novamente'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      toast.error('Erro ao alterar status: ' + (error.response?.data?.error || 'Tente novamente'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleActivateBeta = async (feature: string) => {
    ...
    try {
      ...
      toast.error('Erro ao ativar feature: ' + (error.response?.data?.error || 'Tente novamente'));
    } finally {
      setIsLoading(false);
    }
  };
                ...
                <CardContent>
                  <p className="text-sm text-red-700">
                    ...
                    <ul className="list-disc ml-5 mt-2 space-y-1">
                      <li>Enviar mensagem proativa via Aurora</li>
          ...
          <TabsContent value="growth" className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                />
                <BetaFeatureButton
                  name="Aurora Enhanced"
                  description="IA com contexto completo da loja"
                  onClick={() => handleActivateBeta('aurora-enhanced')}
                  isLoading={isLoading}
                />
              </CardContent>
            </Card>
            ...
            <Card className="border-purple-200 bg-purple-50">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label>Enviar Mensagem via Aurora (Bypass)</Label>
                  <Textarea
                    placeholder="Digite a mensagem que Aurora enviará para o dono..."
                    value={auroraMessage}
